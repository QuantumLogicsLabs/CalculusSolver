...
## 4. Systems Architecture Enhancements (Task B5 Revision)

### 4.1 Four-Component Pipeline Flow
The core deep learning mathematical engine processes inputs across four straightforward components:
- **Tree Encoder**: It reads input math tokens and applies parent-child structural matrix weights to map how expressions are connected.
- **Rule Head**: A dense classification network that maps internal tokens directly into calculus reduction rule IDs.
- **Tree Decoder**: An autoregressive token generator that combines rule embeddings with encoder memory to build output sequences step-by-step using a Beam Search candidates loop.
- **Step Tracer**: A template decoder that reads structural vectors and chooses the exact text template phrase corresponding to the mathematical steps taken.

### 4.2 Symbolic Verification Loop Subprocess
Instead of relying on approximate string matches, every candidate sequence generated by the Beam Search loop triggers an isolated system verification check. The framework runs each beam candidate through a Python subprocess call to `python -m slang.verifier` to check if the generated mathematical operation is valid, blocking systemic token hallucinations.

### 4.3 Standard 3-Stage Structural Training Mechanics
Optimization follows a 3-step structured timeline to ensure stable metrics:
1. **Stage 1 (Sequence Pre-training)**: Trains baseline token prediction on standard calculus sets.
2. **Stage 2 (Supervised Fine-Tuning)**: Fine-tunes model states directly on targeted data configurations, passing golden rule labels through the decoder to keep token alignments stable.
3. **Stage 3 (Constraint-Reinforced Verification)**: Fine-tunes candidate weights using rewards returned directly from the verifier subprocess checks.

### 4.4 Cross-Repository Architecture Layout
The application passes expressions cleanly across repositories and independent runtime environments:

[ Frontend Website UI ]
│  (User inputs math expression string)
▼
...
▼
[ Final Solved Formula + Execution Logs Output JSON ]


### 4.5 How to Add a New Operation (Contributor's Step-by-Step Guide)
Follow these explicit steps to implement a new calculus/algebraic transformation rule into the engine:

1. **Update Token Vocabulary**: Add the token signature of the new operation (e.g., `\"log_base\"`) into `data/vocab.json`.
2. **Implement Structural Relational Mapping**: If the operation introduces new tree constraints, add its structural parent-child configurations inside `model/tree_encoder.py`.
3. **Register Rule Target Classification Labels**: Append the plaintext output string signature mapping into the `labels()` registration list within `model/rule_head.py`.
4. **Append Step Tracer Verification Templates**: Add a corresponding text output sentence structure into the `self.templates` array inside `model/step_tracer.py`.
5. **Expand Test Suites**: Add targeted verification records inside `eval/benchmarks/` and execute validation checks to ensure zero runtime model crashing.
